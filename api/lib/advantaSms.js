 ...
 *
 * Same integration pattern as Machakos (RHNsectPro / partner 7026).
 * For HMIS transactional alerts, prefer sendsms (ADVANTA_USE_OTP_ROUTE=false).
 */
 ...
 * @param {{ mobile: string, message: string }} opts
 */
async function sendAdvantaSms({ mobile, message }) {
  ...
  // Default false for HMIS transactional alerts (OTP route is for login codes).
  const useOtpRoute = String(process.env.ADVANTA_USE_OTP_ROUTE || 'false').toLowerCase() === 'true';
